...
function Today({ users, foods, categories, meal, setMeal }) {
  ...
  useEffect(() => {
    ...
    const settingCals = currentUser ? currentFoods.map(food => food.calories) : null
    const setCategories = currentUser ? setCals(setCurrentCategories(currentUser.categories)) : null
