...
function Welcome( { errors, isLoading } ) {
  ...
  return (
    <div>
      ...
      <br/>
      { currentUser && isLoading === false ? null : <button className="welcome" onClick={clickSignUp}>Signup</button>}
      &nbsp;
      &nbsp;
      &nbsp;
      &nbsp;
      { currentUser && isLoading === false ? null : <button className="welcome" onClick={clickLogin}>Login</button>}
      { errors ? errors.map(error => <br key={error}/>) : null }
